...
namespace CleanWindow
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        Dictionary<string,Texture2D> gameBackgrounds=new Dictionary<string,Texture2D>();//添加背景Dictionary
        ...
        //Dictionary<string, Texture2D> transitionMapLocations = new Dictionary<string, Texture2D>();//添加 transitionMapLocation dictionary
        //Dictionary<string, Texture2D> transitionMapNameTextures = new Dictionary<string, Texture2D>();//添加transitionMapNameTexture dictionary
        ...
        Song bgMusic;
        Dictionary<string, Song> bgMusics = new Dictionary<string, Song>();//添加背景音乐dictionary
        ...
        Texture2D stageTexture;                             //当前关卡提示图片
        Texture2D stageNumberTexture;
        List<Texture2D> keepPhotos = new List<Texture2D>();           //保存每个关卡的照片
        List<Texture2D> keepPhotosName = new List<Texture2D>();       //keep stage photo name
        int fileID = 1;
        string filePath;
        //Texture2D[,] senceTexture = new Texture2D[3, 3];    //游戏背景图片
        Texture2D[] senceTexture = new Texture2D[3];    //游戏背景图片
        Vector2 getHeadPosition = Vector2.Zero;
        double headSholderLength = -1;

        string capturesPath;        //照片存放路径
        float headDepth;
        ...
        {
            float clientBoundsWidth = (float)GraphicsDevice.Viewport.Width;              //获取客户端宽
            float clientBoundsHeight = (float)GraphicsDevice.Viewport.Height;            //获取客户端高
            Console.WriteLine("宽：" + clientBoundsWidth + "  高：" + clientBoundsHeight);
